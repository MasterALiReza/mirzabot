...
🧑‍🦯 شما میتوانید شیوه اتصال را  با فشردن دکمه زیر و انتخاب سیستم عامل خود را دریافت کنید";
    ...
    if (count($dataoutput['configs']) != 1 and $marzban_list_get['config'] == "onconfig") {
        ...
        if ($marzban_list_get['type'] == "WGDashboard") {
            ...
            unlink($urlimage);
            if (isset($marzban_list_get['qr_wgd']) && $marzban_list_get['qr_wgd'] == 'onqrwgd' && !empty($output_config_link)) {
                try {
                    $urlimageqr = "$from_id$randomString.png";
                    $qrCode = createqrcode($output_config_link);
                    file_put_contents($urlimageqr, $qrCode->getString());
                    addBackgroundImage($urlimageqr, $qrCode, $Pathfiles . 'images.jpg');
                    telegram('sendphoto', [
                        'chat_id' => $from_id,
                        'photo' => new CURLFile($urlimageqr),
                        'caption' => "کد QR اتصال",
                        'parse_mode' => "HTML",
                    ]);
                    if (file_exists($urlimageqr)) {
                        unlink($urlimageqr);
                    }
                } catch (Exception $e) {
                    error_log("Failed to generate or send QR code: " . $e->getMessage());
                }
            }
        } else {
            $urlimage = "$from_id$randomString.png";
...
✍️ در صورت درست بودن رسید پرداخت را تایید نمایید.";
    ...
    if ($marzban['status_extend'] == "off_extend") {
        ...
        unset($keyboarddateservies['Extra_time']);
    }
...
▫️زمان خرید : $timejalali";
    ...
    $marzban_list_get = select("marzban_panel", "*", "name_panel", $nameloc['Service_location'], "select");
    if ($marzban_list_get['status_extend'] == "off_extend") {
        sendmessage($from_id, "❌ امکان این عملیات در این پنل وجود ندارد", null, 'html');
        return;
    }
    $DataUserOut = $ManagePanel->DataUser($nameloc['Service_location'], $nameloc['username']);
    if ($DataUserOut['status'] == "Unsuccessful") {
    ...
    $extrapricevalue = $setting['pricevolume'];
    update("user", "Processing_value", $nameloc['id_invoice'], "id", $from_id);
    $user['step'] = "getvolumeextra";
    step('getvolumeextra', $from_id);
    $textextra = "📌 حجم درخواستی خود را برای افزایش سرویس ارسال کنید.
🔔قیمت هر گیگ حجم $extrapricevalue تومان می باشد.";
    $bakinfos = json_encode([
        'inline_keyboard' => [
            [
    ...
    Editmessagetext($from_id, $message_id, $textextra, $bakinfos, 'HTML');
} elseif ($user['step'] == "getvolumeextra") {
    if (!ctype_digit($text)) {
        sendmessage($from_id, "لطفا فقط عدد ارسال کنید", $backuser, 'HTML');
        return;
    }
    if ($text < 1) {
        sendmessage($from_id, "❌ مبلغ وارد شده نامعتبر می باشد", $backuser, 'HTML');
        return;
    }
    ...
    $keyboardsetting = json_encode([
        'inline_keyboard' => [
            [
                ['text' => $textbotlang['users']['extraVolume']['extracheck'], 'callback_data' => 'confirmaextra-' . $text],
            ]
        ]
    ]);
    $priceextra = number_format($priceextra, 0);
    $extrapricevalues = number_format($extrapricevalue, 0);
    $textextra = "🧾 فاکتور افزایش حجم سرویس
 
 مبلغ هر گیگ افزایش حجم $extrapricevalues تومان می باشد.
 مقدار حجم درخواستی شما : $text گیگابایت می باشد.
  
 💰 مبلغ قابل پرداخت $priceextra تومان می باشد.
 
 درصورت تایید بر روی پرداخت کلیک کنید.";
    ...
    $extrapricevalue = $setting['pricevolume'];
    $cost_user = $extrapricevalue * $volume;
    $extrapricevalueBot = $setting['minpricevolume'];
    $cost_bot = $extrapricevalueBot * $volume;

    $botbalance = select("botsaz", "*", "bot_token", $ApiToken, "select");
    $userbotbalance = select("user", "*", "id", $botbalance['id_user'], "select");
    
    if ($cost_bot >= $userbotbalance['Balance'] && $userbotbalance['agent'] != "n2") {
        sendmessage($from_id, "❌ خطایی در خرید رخ داده است برای رفع مشکل با پشتیبانی در ارتباط باشید", $keyboard, 'HTML');
        step("home", $from_id);
        foreach ($admin_ids as $admin) {
            sendmessage($admin, "❌ ادمین عزیز موجودی شما به پایان رسید برای فعالسازی به ربات اصلی مراجعه و ربات خود را شارژ نمایید.", null, 'HTML');
    ...
    if ($cost_user > $user['Balance'] && intval($cost_user) != 0) {
        $minbalance = number_format(json_decode(select("PaySetting", "*", "NamePay", "minbalance", "select")['ValuePay'], true)[$userbot['agent']]);
        $maxbalance = number_format(json_decode(select("PaySetting", "*", "NamePay", "maxbalance", "select")['ValuePay'], true)[$userbot['agent']]);
        $bakinfos = json_encode([
            'inline_keyboard' => [
        ...
        Editmessagetext($from_id, $message_id, "❌ موجودی شما برای پرداخت این فاکتور کافی نمی باشد.
💸  برای افزایش موجودی مبلغ را  به تومان وارد کنید:
✅  حداقل مبلغ $minbalance حداکثر مبلغ $maxbalance تومان می باشد", $bakinfos, 'HTML');
        step('get_price', $from_id);
        return;
    ...
    if ($extra_volume['status'] == false) {
        ...
        $textreports = "
خطای افزایش حجم سرویس در ربات نماینده
نام پنل : {$marzban_list_get['name_panel']}
نام کاربری سرویس : {$nameloc['username']}
دلیل خطا : {$extra_volume['msg']}";
        sendmessage($from_id, "❌خطایی در افزایش حجم سرویس در ربات رخ داده با پشتیبانی در ارتباط باشید", null, 'HTML');
        if (strlen($settingmain['Channel_Report']) > 0) {
            telegram('sendmessage', [
                'chat_id' => $settingmain['Channel_Report'],
                'message_thread_id' => $errorreport,
    ...
    $dateacc = date('Y/m/d H:i:s');
    $type = "extra_user";
    $cost_user_str = strval($cost_user);
    $extra_vol_out = json_encode($extra_volume);
    $stmt->bind_param("sssssss", $from_id, $nameloc['username'], $data_for_database, $type, $dateacc, $cost_user_str, $extra_vol_out);
    $stmt->execute();
    $stmt->close();

    if (intval($cost_user) != 0) {
        $Balance_prim = $user['Balance'] - $cost_user;
        $userbalance = json_decode(file_get_contents("data/$from_id/$from_id.json"), true);
        $userbalance['Balance'] = $Balance_prim;
        file_put_contents("data/$from_id/$from_id.json", json_encode($userbalance));
    }
    ...
    if (intval($userbotbalance['pricediscount']) != 0) {
        $resultper = ($cost_bot * $userbotbalance['pricediscount']) / 100;
        $cost_bot = $cost_bot - $resultper;
    }
    $Balancebot = $userbotbalance['Balance'] - $cost_bot;
    update("user", "Balance", $Balancebot, "id", $userbotbalance['id']);
    ...
    $priceproductformat = number_format($cost_user);
    $textextra = "✅ افزایش حجم با موفقیت انجام شد.

 💰 مبلغ پرداخت شده : $priceproductformat تومان
 مقدار حجم افزوده شده : $volume گیگابایت";
    sendmessage($from_id, $textextra, $keyboardextrafnished, 'HTML');
    step('home', $from_id);
    ...
    $extratimepricevalue = $setting['pricetime'];
    update("user", "Processing_value", $nameloc['id_invoice'], "id", $from_id);
    $user['step'] = "gettimeextra";
    step('gettimeextra', $from_id);
    $textextra = "📌 زمان درخواستی خود را برای افزایش سرویس ارسال کنید.(به صورت روز ارسال کنید.)
🔔قیمت هر روز زمان $extratimepricevalue تومان می باشد.";
    $bakinfos = json_encode([
        'inline_keyboard' => [
            [
                ['text' => $textbotlang['users']['stateus']['backinfo'], 'callback_data' => "product_" . $nameloc['id_invoice']],
    ...
    $marzban_list_get = select("marzban_panel", "*", "name_panel", $nameloc['Service_location'], "select");
    $extratimepricevalue = $setting['pricetime'];
    $priceextra = $extratimepricevalue * $text;
    $keyboardsetting = json_encode([
        'inline_keyboard' => [
            [
                ['text' => "✅ تایید افزایش زمان", 'callback_data' => 'confirmtime-' . $text],
            ]
        ]
    ]);
    $priceextra = number_format($priceextra, 0);
    $extratimepricevalues = number_format($extratimepricevalue, 0);
    $textextra = "🧾 فاکتور افزایش زمان سرویس
 
 مبلغ هر روز افزایش زمان $extratimepricevalues تومان می باشد.
 مقدار زمان درخواستی شما : $text روز می باشد.
 ...
 درصورت تایید بر روی پرداخت کلیک کنید.";
    ...
    $extratimepricevalue = $setting['pricetime'];
    $cost_user = $extratimepricevalue * $time;
    $extratimepricevalueBot = $setting['minpricetime'];
    $cost_bot = $extratimepricevalueBot * $time;
...
✅  حداقل مبلغ $minbalance حداکثر مبلغ $maxbalance تومان می باشد", $bakinfos, 'HTML');
    ...
    $extra_time = $ManagePanel->extra_time($nameloc['username'], $marzban_list_get['code_panel'], $time);
    if ($extra_time['status'] == false) {
        $extra_time['msg'] = json_encode($extra_time['msg']);
        $textreports = "
خطای افزایش زمان سرویس در ربات نماینده
نام پنل : {$marzban_list_get['name_panel']}
نام کاربری سرویس : {$nameloc['username']}
دلیل خطا : {$extra_time['msg']}";
        sendmessage($from_id, "❌خطایی در افزایش زمان سرویس در ربات رخ داده با پشتیبانی در ارتباط باشید", null, 'HTML');
        if (strlen($settingmain['Channel_Report']) > 0) {
            telegram('sendmessage', [
                'chat_id' => $settingmain['Channel_Report'],
                'message_thread_id' => $errorreport,
    ...
    $cost_user_str = strval($cost_user);
    $extra_time_out = json_encode($extra_time);
    $stmt->bind_param("sssssss", $from_id, $nameloc['username'], $data_for_database, $type, $dateacc, $cost_user_str, $extra_time_out);
    $stmt->execute();
    $stmt->close();
    ...
    $priceproductformat = number_format($cost_user);
    $textextra = "✅ افزایش زمان با موفقیت انجام شد.

 💰 مبلغ پرداخت شده : $priceproductformat تومان
 مقدار زمان افزوده شده : $time روز";
    sendmessage($from_id, $textextra, $keyboardextrafnished, 'HTML');
    step('home', $from_id);
