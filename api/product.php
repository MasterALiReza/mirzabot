...
switch ($data['actions'] ?? '') {
    ...
    case 'products':
        ...
        try {
            ...
            $query = "SELECT * FROM product WHERE (id  LIKE CONCAT('%', :id_product, '%') OR name_product  LIKE CONCAT('%', :name_product, '%')) ORDER BY CAST(Volume_constraint AS UNSIGNED) ASC, CAST(price_product AS UNSIGNED) ASC, id ASC LIMIT :limit OFFSET :offset";
